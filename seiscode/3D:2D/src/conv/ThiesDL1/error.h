/*! \file error.h
 * \brief error handling and exceptions (prototypes)
 * 
 * ----------------------------------------------------------------------------
 * 
 * $Id$
 ...
 * \date 25/11/2008
 * 
 * error handling and exceptions (prototypes)
 * 
 ...
 */
  ...
  class ExceptionTimeOut:
    public dl1::Exception {
      public:
        //! Create with message, failed assertion, and code position
        ExceptionTimeOut(const char* message, 
                  const char* file,
                  const int& line,
                  const char* condition,
                  const int& timeout):
          Exception(message, file, line, condition), Mtimeout(timeout) { }
        virtual ~ExceptionTimeOut() { }
        virtual void report() const;
      private:
 ...
 * \param M message of type char*
 */
#define DL1_assert(C,M) \
  DL1_Xassert( C , M , dl1::Exception )
 ...
 */
#define DL1_abort(M) \
  throw( dl1::Exception ( M , __FILE__, __LINE__ )) 

#define DL1_illegal DL1_abort("illegal call!")
 ...
 */
#define DL1_report_assert(C,M,V) \
  if (!(C)) { \
    dl1::report_violation(M, __FILE__, __LINE__, #C); \
...
/*! \brief Macro to throw timeout exception
 *
 * \ingroup group_error
 * \param M message of type char*
 * \param T timeout value
 */
#define DL1_timeout( M, T ) \
  throw( dl1::ExceptionTimeOut( M , __FILE__, __LINE__, 0, T )); 
