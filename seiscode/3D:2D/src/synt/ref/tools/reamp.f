c this is <reamp.f> originally by J. Ungerer 1990
c======================================================================
c
...
c----------------------------------------------------------------------
c 
c this is a special version to write matrix amplitudes for any source
c
c======================================================================
c
c REVISIONS and CHANGES (only major changes are reported in detail)
c
c 22/02/1997   V1.0   * copied from resus.f (V1.2)
c           
c======================================================================
      PROGRAM resus

      character*70 version
      parameter(version='REAMP   V1.0   Reflectivity Amplitudes')
...
c amplitude matrix
      integer omout, uout
      parameter(omout=205,uout=205)
      real ampb(3,omout,uout), ampd(3,omout,uout)
      integer nomout, nuout, iomout, iuout, stepomout, stepuout
c matrix reading
...
c sff FREE
      ...
      parameter(sff_maxfree=100+2*ME)
...
c here are the keys to our commandline options
      ...
      data cl_optarg/1h-,1h1,10hanswer.amp,1h-,2h  ,1h-/
...
c give basic information
     ...
     &  ME, 0, MSL, Mf, sff_maxfree, hfktstr)
      ...
      print *,' '
      print *,'NOTICE: This is NOT REFMET this is REAMP'
      print *,'NOTICE: To calculate amplitudes I have to read a matrix file'
      print *,' '
c      call buggy
...
c init output field
      do iomout=1,omout
        do iuout=1,uout
          do i=1,3
            ampb(i,iomout,iuout)=0.
            ampd(i,iomout,iuout)=0.
          enddo
        enddo
      enddo
...
c----------------------------------------------------------------------
c
c calculate output stepsize
      stepomout=0
      stepuout=0
  211 stepomout=stepomout+1
      nomout=(fma-fmi+1)/stepomout+1
      if (nomout.gt.omout) goto 211
  212 stepuout=stepuout+1
      nuout=nu/stepuout+1
      if (nuout.gt.uout) goto 211
...
c----------------------------------------------------------------------
c set output index
      iomout=int((f-fmi+1)/stepomout)+1
      iuout=int(l/stepuout)+1
c 
...
c catch output
      do k=0,2
        ampb(k+1,iomout,iuout)=sngl(abs(b0(k)))+ampb(k+1,iomout,iuout)
        ampd(k+1,iomout,iuout)=sngl(abs(d0(k)))+ampd(k+1,iomout,iuout)
      enddo
...
c write output
      ...
      if (cl_vlevel.gt.lev2) print 52,'opening ',
     &    basename(1:index(basename, ' ')-1)
      open(20, basename, form='unformatted', err=99)

      write(20, err=95) nomout,nuout,df,du,stepomout,stepuout,fmin,umin
      write(20, err=95) (((ampb(i,k,l), i=1,3), k=1,nomout), l=1,nuout)
      write(20, err=95) (((ampd(i,k,l), i=1,3), k=1,nomout), l=1,nuout)
