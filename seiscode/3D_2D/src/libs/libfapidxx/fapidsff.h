/*! \file fapidsff.h
 * \brief This is the interface usually expected by Fortran programs linking to libsff.a (prototypes)
 * 
 * ----------------------------------------------------------------------------
 * 
...
/*----------------------------------------------------------------------*/
// provide Fortran types

// this is the interface declaration
// the function should be callable from Fortran
extern "C" {
...
#endif
// this header is only for internal use within libfapidxx
#error "this header file must only be used internally!"
#endif
...
extern int sff_close__(integer *lu, integer *ierr);

extern int sff_select_format__(char *formatid, integer *ierr, ftnlen
                               formatid_len);

extern int sff_select_input_format__(char *formatid, integer *ierr, ftnlen
                                     formatid_len);

extern int sff_select_output_format__(char *formatid, integer *ierr, ftnlen
                                      formatid_len);

extern int sff_help_formats__(void);

extern int sff_help_details__(void);
...
#endif // TF_FAPIDSFF_H_VERSION (includeguard)
