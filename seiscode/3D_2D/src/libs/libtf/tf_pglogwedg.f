...
c
c This is a modified version to be used with logarithmic scales
c
c REVISIONS and CHANGES
c    25/08/99   V1.0   Thomas Forbriger
c    17/12/07   V1.1   initialize internal variable to satisfy gfortran
c
c==============================================================================
cS
c
...
C+
      SUBROUTINE TF_PGLOGWEDG(SIDE, DISP, WIDTH, FG, BG, LABEL)
...
C
      IF(NSIDE.EQ.BOT) THEN
        CALL PGBOX('BCNSTL',0.0,0,'BC',0.0,0)
      ELSE IF(NSIDE.EQ.TOP) THEN
        CALL PGBOX('BCMSTL',0.0,0,'BC',0.0,0)
      ELSE IF(NSIDE.EQ.LFT) THEN
        CALL PGBOX('BC',0.0,0,'BCNSTL',0.0,0)
      ELSE IF(NSIDE.EQ.RGT) THEN
        CALL PGBOX('BC',0.0,0,'BCMSTL',0.0,0)
      ENDIF
C
