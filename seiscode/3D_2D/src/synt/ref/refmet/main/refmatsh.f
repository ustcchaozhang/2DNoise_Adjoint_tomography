...
c======================================================================
      ...
      parameter(version='REFMAT   V1.0   Reflectivity Matrix')
...
c     &          K0(ME),K1(ME),K2(ME),K3(ME),L1(ME),L2(ME)
...
c     &            DDu,ea,eb,earg,red,g(MSL)
c     &            In1,In2,In3,In4,In5,In6,
c     &            AQ(0:2),BQ(0:2),CQ(0:2),DQ(0:2),EQ(2),FQ(2),
c     &            B0(0:2),D0(0:2),F0(2),
     &            E11(0:MS),E22(0:MS)

c      COMPLEX*16  IDFr,IDFphi,IDFz,
c     &            IDNr,IDNphi,IDNz,
c     &            VFr(MSL,ME),VFphi(MSL,ME),VFz(MSL,ME),
c     &            VNr(MSL,ME),VNphi(MSL,ME),VNz(MSL,ME)

      COMPLEX     C1(MSL),C2(MSL),C3(MSL),
...
c here are the keys to our commandline options
      ...
      data cl_optarg/1h-,1h1,10hrefmet.out,1h-,2h  ,1h-/
...
c      hfkt(tvar,T1,T2)=0.75*pi/(T2-T1)*dsin(pi*(tvar-T1)/(T2-T1))**3
c      hfktstr='hfkt(tvar,T1,T2)=0.75*pi/(T2-T1)*dsin(pi*(tvar-T1)/(T2-T1))**3'
CC    smooth blow
...
c set options
      call tflib_cmdline(1, cl_lastarg,
     &     cl_maxopt, cl_optid, cl_optarg, cl_optset, cl_opthasarg)
      ...
      writematrix=cl_optset(6)
      if (writematrix) matrixfile=cl_optarg(6)
      cl_rprogress=.false.
...
c      if (typ.eq.2) then
c        line='TTNZNRNTFZFRFT'//cl_comsel
c        cl_comsel=line
c        print *,'NOTICE: set component selection to ',
c     &    cl_comsel(1:(index(cl_comsel, ' ')-1))
c        print *,'NOTICE: because of vertical single force'
c      endif
c      if (cl_debug) print *,'DEBUG cl_comsel ',cl_comsel
...
c         DO 184 f=1,SL
c            VFr(f,E)  =dcmplx(0.,0.)
c            VFphi(f,E)=dcmplx(0.,0.)
c            VFz(f,E)  =dcmplx(0.,0.)
c            VNr(f,E)  =dcmplx(0.,0.)
c            VNphi(f,E)=dcmplx(0.,0.)
c            VNz(f,E)  =dcmplx(0.,0.)
c  184    CONTINUE
c  190 CONTINUE
...
c      DO 210 f=fmi,fma
c         IF(fmin.LE.fr(f) .and. fr(f).LT.fwil) THEN
c            ftap(f)=0.5D0*(1.D0-dcos(pi*(fr(f)-fmin)/(fwil-fmin)))
c         ELSE IF(fwil.LE.fr(f) .and. fr(f).LE.fwir) THEN
c            ftap(f)=1.D0 
c         ELSE IF(fwir.LT.fr(f) .and. fr(f).LE.fmax) THEN
c            ftap(f)=0.5D0*(1.D0+dcos(pi*(fr(f)-fwir)/(fmax-fwir)))
c         ENDIF
c  210 CONTINUE
...
c        phiB(E)=2.D0*pi*phi(E)/360.D0
c
C       Winkel- und Tensorparameter
c        K0(E)=Mzz
c        K1(E)=Mxz*dcos(phiB(E))+Myz*dsin(phiB(E))
c        K2(E)=0.5D0*(Myy-Mxx)*dcos(2.D0*phiB(E))-Mxy*dsin(2.D0*phiB(E))
c        hilf=Mxx*dcos(phiB(E))**2.D0 + Myy*dsin(phiB(E))**2.D0
c        K3(E)=hilf + Mxy*dsin(2.D0*phiB(E))
c        L1(E)=Mxz*dsin(phiB(E))-Myz*dcos(phiB(E))
c        L2(E)=0.5D0*(Myy-Mxx)*dsin(2.D0*phiB(E))+Mxy*dcos(2.D0*phiB(E))
c      else
...
c
c      if (radius.gt.0.d0) then
c
c determine exponent
c        if (typ.eq.1) then
c          correx=2
c        elseif (typ.eq.2) then
c          correx=1
c        elseif (typ.eq.3) then
c          correx=1
c        else
c          correx=0
c        endif
c do it
c        do e=1,ne
c           delta=max(r(E),0.001d0)
c           delta=delta/radius
c  221      if (delta.gt.pi) then
c             delta=delta-2.d0*pi
c             goto 221
c           endif
c  222      if (delta.lt.(-pi)) then
c             delta=delta+2.d0*pi
c             goto 222
c           endif
c           if (sin(delta).eq.0.d0) then
c             print *,'ERROR: at receiver #',e,' dist ',r(e)
c             stop 'ERROR: amplitude correction breaks down'
c           endif
c           ampcorr(e)=((radius/(radius-ZQ))**correx)*sqrt(delta/sin(delta))
c        enddo
c      endif
...
c----------------------------------------------------------------------
c save depth of source layer 
      write(matrixlu, err=95) z(h)
...
c        DO 270 f=1,SL
c           IF(t(f).LT.The) THEN
c             g(f)=DCMPLX(0.)
c           ELSE IF(t(f).GE.The .and. t(f).LE.The+Thd) THEN
c             g(f)=DCMPLX(hfkt(t(f),The,The+Thd))*ap
c           ELSE IF(t(f).GT.The+Thd) THEN
c             g(f)=DCMPLX(0.)
c           ENDIF
c  270   CONTINUE
c
C       Schnelle Fouriertransformation:Aufruf von DFORK
c        CALL DFORK(SL,g,hin)
c      ENDIF
c
...
c         IF(outsig.EQ.1) THEN
c           fr(1)=1.D-30
c           w(1)=2.D0*pi*fr(1)
c           g(f)=g(f)/(IME*w(f)) *ftap(f)
C        Anregung fuer Bodengeschwindigkeit
c         ELSE IF(outsig.EQ.2) THEN
c           g(f)=g(f) *ftap(f)
C        Anregung fuer Bodenbeschleunigung(spektrale Differentiation)
c         ELSE IF(outsig.EQ.3) THEN
c           g(f)=g(f)*IME*w(f) *ftap(f)
c         ENDIF
c  280 CONTINUE
...
c      sff_nfree=min(sff_maxfree, sff_nfree+1)
c      sff_free(sff_nfree)=' '
c      sff_nfree=min(sff_maxfree, sff_nfree+1)
c      sff_free(sff_nfree)='receiver configuration:'
c      if (radius.gt.0.d0) then 
c        sff_nfree=min(sff_maxfree, sff_nfree+1)
c        write(sff_free(sff_nfree), 380) 'receiver','epicentral distance [km]',
c     &    'azimuth angle [degrees]','amp. corr.'
c        do e=1,ne
c          sff_nfree=min(sff_maxfree, sff_nfree+1)
c          write(sff_free(sff_nfree), 390) e, r(e), phi(e), ampcorr(E)
c        enddo
c  380   format(2x,a10,1x,a25,1x,a25,1x,a12)
c  390   format(2x,i10,1x,f25.6,1x,f25.3,1x,f12.4)
c      else
c        sff_nfree=min(sff_maxfree, sff_nfree+1)
c        write(sff_free(sff_nfree), 381) 'receiver','epicentral distance [km]',
c     &    'azimuth angle [degrees]'
c        do e=1,ne
c          sff_nfree=min(sff_maxfree, sff_nfree+1)
c          write(sff_free(sff_nfree), 391) e, r(e), phi(e)
c        enddo
c  381   format(2x,a10,1x,a25,1x,a25)
c  391   format(2x,i10,1x,f25.6,1x,f25.3)
c      endif
...
c        sff_nfree=min(sff_maxfree, sff_nfree+1)
c        sff_free(sff_nfree)=
c     &    'amplitude correction factor according to flat earth approximation:'
c        sff_nfree=min(sff_maxfree, sff_nfree+1)
c        write(sff_free(sff_nfree), 5050) correx
c 5050   format("(R/(R-ZQ))**",i1,
c     &    "*sqrt(delta/sin(delta))   delta: epicentral distance")
c      else
c        sff_nfree=min(sff_maxfree, sff_nfree+1)
c        sff_free(sff_nfree)=' '
c        sff_nfree=min(sff_maxfree, sff_nfree+1)
c        sff_free(sff_nfree)=
c     &    'there is no amplitude correction applied as this is a flat model'
c      endif
...
c      IF (umin.LE.u .and. u.LT.uwil) THEN
c         utap=0.5D0*(1.D0-dcos(pi*(u-umin)/(uwil-umin)))
c      ELSE IF(uwil.LE.u .and. u.LE.uwir) THEN
c         utap=1.D0
c      ELSE IF (uwir.LT.u .and. u.LE.umax) THEN
c         utap=0.5D0*(1.D0+dcos(pi*(u-uwir)/(umax-uwir)))
c      ENDIF

C     Gewichtungsfaktor fuer Trapezregel.Der erste und letzte Funk-
C     tionswert werden nur halb gewichtet(gew=0.5)
c      gew=1.D0
c      IF ( l.EQ.1  .OR. l.EQ.Nu ) gew=0.5D0
C     getaperte und gewichtete Integrationsschrittweite fuer Trapezregel
c      DDu=DCMPLX(Du*gew*utap)
...
c         CALL MATMUL(RTD11(i+1),RTD12(i+1),RTD21(i+1),RTD22(i+1),
c     &               Rppu(i+1) ,Rspu(i+1) ,Rpsu(i+1) ,Rssu(i+1) ,
c     &               H11       ,H12       ,H21       ,H22       )
cc         H11=1.D0-H11
c         H12=  -H12
c         H21=  -H21
cc         H22=1.D0-H22
c         CALL MATINV(H11,H12,H21,H22, I11,I12,I21,I22)
c         CALL MATMUL(Tppu(i+1),Tspu(i+1),Tpsu(i+1),Tssu(i+1),
c     &               I11      ,I12      ,I21      ,I22      ,
c     &               H11      ,H12      ,H21      ,H22      )         
c         CALL MATMUL(H11       ,H12       ,H21       ,H22       ,
c     &               RTD11(i+1),RTD12(i+1),RTD21(i+1),RTD22(i+1),
c     &               I11       ,I12       ,I21       ,I22       )
c         CALL MATMUL(I11      ,I12      ,I21      ,I22      ,
c     &               Tppd(i+1),Tspd(i+1),Tpsd(i+1),Tssd(i+1),
c     &               H11      ,H12      ,H21      ,H22      )
c         H11=Rppd(i+1)+H11
c         H12=Rspd(i+1)+H12
c         H21=Rpsd(i+1)+H21
c         H22=Rssd(i+1)+H22
c         RTD11(i)=E11(i)*E11(i)*H11
c         RTD12(i)=E11(i)*E22(i)*H12
c         RTD21(i)=E11(i)*E22(i)*H21
c         RTD22(i)=E22(i)*E22(i)*H22
...
c         CALL MATMUL (Rppd(i+1),Rspd(i+1),Rpsd(i+1),Rssd(i+1),
c     &                RBU11(i) ,RBU12(i) ,RBU21(i) ,RBU22(i) ,
c     &                H11      ,H12      ,H21      ,H22      )
c         H11 =1.D0-H11
c         H12 =  -H12
c         H21 =  -H21
...
c         H22 =E22(i)*I22
c         CALL MATMUL(H11      ,H12      ,H21      ,H22      ,
c     &               Tppu(i+1),Tspu(i+1),Tpsu(i+1),Tssu(i+1),
c     &               S11(i)   ,S12(i)   ,S21(i)   ,S22(i)   )
c         CALL MATMUL(TTUo11(i),TTUo12(i) ,TTUo21(i)   ,TTUo22(i)  ,
c     &               S11(i)   ,S12(i)    ,S21(i)      ,S22(i)     ,
c     &             TTUo11(i+1),TTUo12(i+1),TTUo21(i+1),TTUo22(i+1)) 
C        Ende Rekursion TTUo 


C        Rekursion fuer RTU(i) [liefert auch RBU(i) fuer TTUo(i)]
c         CALL MATMUL(RBU11(i) ,RBU12(i) ,RBU21(i) ,RBU22(i) ,
c     &               Rppd(i+1),Rspd(i+1),Rpsd(i+1),Rssd(i+1),
c     &               H11      ,H12      ,H21      ,H22      )
c         H11=1.D0-H11
c         H12=  -H12
c         H21=  -H21
c         H22=1.D0-H22
c         CALL MATINV(H11,H12,H21,H22,I11,I12,I21,I22)
c         CALL MATMUL(Tppd(i+1),Tspd(i+1),Tpsd(i+1),Tssd(i+1),
c     &               I11,I12,I21,I22,       H11,H12,H21,H22 ) 
c         CALL MATMUL(H11     ,H12     ,H21     ,H22     ,
c     &               RBU11(i),RBU12(i),RBU21(i),RBU22(i),
c     &               I11     ,I12     ,I21     ,I22     )
c         CALL MATMUL(I11      ,I12      ,I21      ,I22      ,
c     &               Tppu(i+1),Tspu(i+1),Tpsu(i+1),Tssu(i+1),
c     &               H11      ,H12      ,H21      ,H22      )
c         RTU11(i+1)=Rppu(i+1)+H11
c         RTU12(i+1)=Rspu(i+1)+H12
c         RTU21(i+1)=Rpsu(i+1)+H21
c         RTU22(i+1)=Rssu(i+1)+H22

C        neue RBU werden auch gebraucht zur Rekursion von TTUo
c         RBU11(i+1)=E11(i+1)*E11(i+1)*RTU11(i+1)
c         RBU12(i+1)=E11(i+1)*E22(i+1)*RTU12(i+1)
c         RBU21(i+1)=E11(i+1)*E22(i+1)*RTU21(i+1)
c         RBU22(i+1)=E22(i+1)*E22(i+1)*RTU22(i+1) 
...
c        AQ(0)=IME*a(h)*ea
c        AQ(1)=2.D0*u*ea
c        AQ(2)=IME*u*u/a(h)*ea 
c        CQ(0)=IME*u*eb
c        CQ(1)=(u*u/b(h)-b(h))*eb
c        CQ(2)=-IME*u*eb
c        EQ(1)=eb/(betaC(h)*betaC(h))
c        EQ(2)=EQ(1)*IME*u/b(h)
c
c        BQ(0)=IME*a(h)/ea
c        BQ(1)=-2.D0*u/ea
c        BQ(2)=IME*u*u/(a(h)*ea)
c        DQ(0)=-IME*u/eb
c        DQ(1)=(u*u/b(h)-b(h))/eb
c        DQ(2)=IME*u/eb
c        FQ(1)=-1.D0/(betaC(h)*betaC(h)*eb)
c        FQ(2)=-FQ(1)*IME*u/b(h)
...
C     Matrizenreihe
c      CALL MATMUL(RM11,RM12,RM21,RM22,RP11,RP12,RP21,RP22,
c     &            H11,H12,H21,H22)
c      H11=1.D0-H11
c      H12=  -H12
c      H21=  -H21
...
c      CALL MATMUL(TP11,TP12,TP21,TP22,I11,I12,I21,I22,
c     &            H11,H12,H21,H22)
...
c         I11=BQ(i)+RM11*AQ(i)+RM12*CQ(i)
c         I22=DQ(i)+RM21*AQ(i)+RM22*CQ(i)
C        Multiplikation der Matrix [H11,H12,H21,H22] mit [I11,I22]
c         B0(i)=H11*I11+H12*I22
c         D0(i)=H21*I11+H22*I22
c 1300 CONTINUE
...
c      DO 1400 k=1,2
c         F0(k)=tp/(1.D0-rm*rp)*(FQ(k)+rm*EQ(k))
c 1400 CONTINUE
...
C**********************************************************************C
c      In1=u*B0(0)+b(0)*D0(0)
c      In2=u*B0(2)+b(0)*D0(2)
c      In3=u*B0(1)+b(0)*D0(1)
c      In4=a(0)*B0(0)-u*D0(0)
c      In5=a(0)*B0(2)-u*D0(2)
c      In6=a(0)*B0(1)-u*D0(1)
c      if (typ.eq.1) then
c moment tensor source
c        FFI=w(f)*w(f)/(4.D0*pi*rho(h))
c      elseif (typ.eq.2) then
c single vertical force
c        FFI=w(f)/(4.d0*pi*rho(h)) 
c      endif
...
c        hilf=J1*(K0(E)*In1+K3(E)*In2)
c        IDFr= FFI*u*(K1(E)*In3*J0-hilf)
c        IDFphi=-FFI*u*(F0(1)*L1(E)*J0+F0(2)*L2(E)*J1)
c        hilf=J0*(K0(E)*In4+K3(E)*In5)
c        IDFz=FFI*IME*u*(hilf+K1(E)*In6*J1)

C     Berechnung der Nahfeld-Integrandenglieder
c        NFI=w(f)/(r(E)*4.D0*pi*rho(h))
c        H11=  (F0(1)-In3)*J1
c        H22=2.D0*(F0(2)-In2)*J2
c        IDNr  =NFI*(K1(E)*H11+K2(E)*H22)
c        IDNphi=NFI*(L1(E)*H11+L2(E)*H22)
c        IDNz=NFI*IME*2.D0*K2(E)*In5*J1

c      elseif (typ.eq.2) then

c for a vertical single force
c        IDFr=-FFI*In1*J1
c        IDFz=FFI*IME*In4*J0
c   no tangential component, no near field
c        IDFphi=dcmplx(0.d0)
c        IDNphi=dcmplx(0.d0)
c        IDNr=dcmplx(0.d0)
c        IDNz=dcmplx(0.d0)
...
c      VFr(f,E)  =VFr(f,E)  +  IDFr*DDu
c      VFphi(f,E)=VFphi(f,E)+IDFphi*DDu
c      VFz(f,E)  =VFz(f,E)  +  IDFz*DDu

C     Aufsummierung zu Nahfeldspektren
c      VNr(f,E)  =VNr(f,E)  +  IDNr*DDu
c      VNphi(f,E)=VNphi(f,E)+IDNphi*DDu
c      VNz(f,E)  =VNz(f,E)  +  IDNz*DDu
...
c
      if (writematrix) then
        write(matrixlu, err=95) (C1(f), C2(f)
     &                           , f=fmi,fma)
      endif
...
c         DO 5000 f=fmi,fma
C           Fernfeld-Spektren 
c            VFr(f,E)  =M0dim*VFr(f,E)*g(f)
c            VFphi(f,E)=M0dim*VFphi(f,E)*g(f)
c            VFz(f,E)  =M0dim*VFz(f,E)*g(f)
C           Nahfeld-Spektren
c            VNr(f,E)  =M0dim*VNr(f,E)*g(f)
c            VNphi(f,E)=M0dim*VNphi(f,E)*g(f)
c            VNz(f,E)  =M0dim*VNz(f,E)*g(f)
c 5000    CONTINUE
c 5100 CONTINUE
...
c           DO f=fmi,fma
c             Fernfeld-Spektren 
c              VFr(f,E)  =VFr(f,E)  *ampcorr(E)
c              VFphi(f,E)=VFphi(f,E)*ampcorr(E)
c              VFz(f,E)  =VFz(f,E)  *ampcorr(E)
c             Nahfeld-Spektren
c              VNr(f,E)  =VNr(f,E)  *ampcorr(E)
c              VNphi(f,E)=VNphi(f,E)*ampcorr(E)
c              VNz(f,E)  =VNz(f,E)  *ampcorr(E)
c           ENDDO
c        ENDDO
c      endif
...
c           help=DSQRT(ZQ*ZQ+r(E)*r(E))/Vred
c         ENDIF
c         DO 5110 f=1,SL
C           Berechnung der Reduktionszeit tred(f,E)
c            tred(f,E)=t(f)+help+Tli-Tre
C           und des Reduktionsfaktors red
c            red=EXP(IME*w(f)*(help+Tli-Tre))
C           Reduktion der Fernfeld-Spektren
c            VFr(f,E)  =VFr(f,E)*red
c            VFphi(f,E)=VFphi(f,E)*red
c            VFz(f,E)  =VFz(f,E)*red
C           Reduktion der Nahfeld-Spektren
c            VNr(f,E)  =VNr(f,E)*red
c            VNphi(f,E)=VNphi(f,E)*red
c            VNz(f,E)  =VNz(f,E)*red
c 5110    CONTINUE
c 5120 CONTINUE
c
...
c      DO 5300 E=1,NE
c         DO 5200 f=0,SL/2-2
c            VFr(SL-f,E)  =DCONJG(VFr(f+2,E))
c            VFphi(SL-f,E)=DCONJG(VFphi(f+2,E))
c            VFz(SL-f,E)  =DCONJG(VFz(f+2,E))
c            VNr(SL-f,E)  =DCONJG(VNr(f+2,E))
c            VNphi(SL-f,E)=DCONJG(VNphi(f+2,E))
c            VNz(SL-f,E)  =DCONJG(VNz(f+2,E))
c 5200    CONTINUE
c 5300 CONTINUE
...
c         CALL DFORK(SL,C1,rueck)
c         CALL DFORK(SL,C2,rueck)
c         CALL DFORK(SL,C3,rueck)
c         CALL DFORK(SL,C4,rueck)
c         CALL DFORK(SL,C5,rueck)
c         CALL DFORK(SL,C6,rueck)                
...
c
c         DO 6600 f=1,SL
c            VFr(f,E)  =C1(f)*1.D0/ap
c            VFphi(f,E)=C2(f)*1.D0/ap
c            VFz(f,E)  =C3(f)*1.D0/ap
c            VNr(f,E)  =C4(f)*1.D0/ap
c            VNphi(f,E)=C5(f)*1.D0/ap
c            VNz(f,E)  =C6(f)*1.D0/ap
c 6600    CONTINUE
c 7000 CONTINUE

c      if (cl_debug) print *,'DEBUG: sff_maxfree, sff_nfree ',
c     &  sff_maxfree, sff_nfree
c======================================================================
c
c output to file
c
c      if (cl_comeach) then
c        call refmet_outputc(basename, fdata, idata, MSL, SL, ME, NE, outunits,
c     &    cl_comsel,
c     &    sff_free, sff_maxfree, sff_nfree, VFz, VFr, VFphi, VNz, VNr, VNphi,
c     &    ZQ, The, Thd, typ, r, phi, dt, tred, cl_vlevel, cl_debug, lev2)
c      else
c        call refmet_outputr(basename, fdata, idata, MSL, SL, ME, NE, outunits,
c     &    cl_comsel,
c     &    sff_free, sff_maxfree, sff_nfree, VFz, VFr, VFphi, VNz, VNr, VNphi,
c     &    ZQ, The, Thd, typ, r, phi, dt, tred, cl_vlevel, cl_debug, lev2)
c      endif
