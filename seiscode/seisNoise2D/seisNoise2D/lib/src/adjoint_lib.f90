...
subroutine misfit_adj_AD(measurement_type,d,s,NSTEP,deltat,f0,&
        ...
        compute_adjoint,adj,num,misfit,tshift,Tc,Te,irec,err_CC)
    ...
    real(kind=CUSTOM_REAL), intent(out),optional :: misfit,tshift
    ! error
    ...
    case ("CC")
        ...
        call CC_misfit(d,s,NSTEP,deltat,tstart,tend,taper_percentage,taper_type, &
            'dt',compute_adjoint,adj,num,misfit,tshift,Tc,Te,irec,err_CC)
    case ("AM")
    ...
    case ("MT")
        ...
        call MT_misfit(d,s,NSTEP,deltat,f0,tstart,tend,taper_percentage,taper_type, &
            'MT',compute_adjoint,adj,num,misfit,tshift,Tc,Te,irec)
    case ("MA")
...
subroutine CC_misfit(d,s,npts,deltat,tstart,tend,taper_percentage,taper_type,&
        ...
        adj,num,misfit,tshift,Tc,Te,irec,err_CC)
    ...
    real(kind=CUSTOM_REAL), dimension(:), allocatable :: tas
    real(kind=CUSTOM_REAL), dimension(npts) :: d_tw,s_tw 
    real(kind=CUSTOM_REAL), dimension(npts) :: cc_function
    ! cc 
    integer :: ishift
    ...
    cc_function=0.0_CUSTOM_REAL
    !!window
    !call cc_window(s,npts,3,i_tstart,i_tend,0,0.d0,nlen,s_tw)
    !call cc_window(d,npts,3,i_tstart,i_tend,0,0.d0,nlen,d_tw)
 
    !print *,'Tc=',Tc,'Te=',Te,'irec=',irec
    ...
    !! CC misfit
    call xcorr_calc(d_tw,s_tw,npts,1,nlen,ishift,dlnA,cc_max,cc_function) ! T(d-s)

    !! by chao, cc_max should be larger than 0.85
    !if (cc_max<=0.85) then
    !   ishift=2000 
    !endif
    ...
    if(misfit_type=='dt') then 
        ...
        !! by chao
        !tshift=1.000000       
        !print *,'err_dt_cc=',err_dt_cc,'cc_max=',cc_max,'tshift=',tshift

        ! output the deltat
        ! write(IOUT,*) tshift
        !! by chao misfit = (deltat/err_CC)**2 
        misfit=(tshift/err_CC)**2

        !misfit=tshift/err_CC
    ...
    if(COMPUTE_ADJOINT) then
        if(misfit_type=='dt') then
            ...
            !adj_tw(1:nlen)=-1.0*s_tw_vel(1:nlen)/Mtr/err_CC**2
        elseif(misfit_type=='am') then 
            ! constant on the bottom
    ...
    if(DISPLAY_DETAILS) then
        ...
        !!! change the number to str  
        write( Tc_str, '(I3)' )  floor(Tc)
        write( irec_str, '(I3)' )  irec
        write(filename,'(a)') &
                trim(output_dir)//trim(adjustl(irec_str))//'_'//trim(adjustl(Tc_str))//'_dat_syn_win'


        open(1,file=trim(filename),status='unknown')
        do  i = i_tstart,i_tend
        !print *,'cc_function(i)=',cc_function(i)
        write(1,'(I5,7e15.5,F9.3)') i,d(i),s(i),tas(i),d_tw(i),s_tw(i),adj(i),cc_function(i),tshift
        !print *,i,d(i),s(i),tas(i), d_tw(i),s_tw(i)
        enddo
        

      !  write(filename,'(a)') &
      !          trim(output_dir)//trim(adjustl(irec_str))//'_'//trim(adjustl(Tc_str))//'_adj_CC_function'
      !  open(2,file=trim(filename),status='unknown')
      !   write(2,'(F6.2)') cc_function(:)
        ...
        if( DISPLAY_DETAILS) then
            if(misfit_type=='dt') then
                open(1,file=trim(output_dir)//trim(adjustl(irec_str))//'_'//trim(adjustl(Tc_str))//'_adj_CC_win',status='unknown')
            elseif(misfit_type=='am') then
                open(1,file=trim(output_dir)//'/adj_AM_win',status='unknown')
            endif
            do  i =  i_tstart,i_tend
            write(1,'(I5,4e15.5)') i,adj(i),cc_max,tshift,err_dt_cc
            enddo
            !close(1)
        endif
        ...
        print *,'deltat fot CC is:',tshift
        print *,'cc_max=',cc_max,'num=',num,'err_dt_cc=',err_dt_cc
        print *,'misfit fot CC is:',(tshift/err_dt_cc)**2

            ! ---------------------------------------------
            if ( (cc_max < CC_MIN) ) then
            ! zero the CC measurments
            ! zero the CC measurments,adj=0,deltat=0.000001,num--
             tshift = 40.0
             misfit = 0.0
             adj(i_tstart:i_tend) = 0.0
             num=num-1
            endif
            ! ---------------------------------------------
    ! output tshift
...
subroutine MT_misfit(d,s,npts,deltat,f0,tstart,tend,taper_percentage,taper_type,&
        misfit_type,compute_adjoint,&
        adj,num,misfit,tshift,Tc,Te,irec)
    ...
    real(kind=CUSTOM_REAL) :: df,df_new,dw,fshort,flong
    ...
    integer :: ictaper
    real(kind=CUSTOM_REAL), dimension(NPT) :: dtau_w, dlnA_w,err_dtau_mt,err_dlnA_mt,wp_taper,wq_taper,w_taper
    complex(CUSTOM_REAL), dimension(NPT) :: trans_func
    real(kind=CUSTOM_REAL) :: err_CC
    ! misfit & adjoint
    real(kind=CUSTOM_REAL) :: misfit_p, misfit_q,dtau_w_mean
    ...
    !print *,'f0,f0/2.5,f0*2.5=',f0,f0/2.5,f0*2.5
    fshort=1.0/Te
    flong=1.0/Tc               
    !print *,'Tshort,Tlong=',1.0/flong,1.0/fshort

    call frequency_limit(s_tw,nlen,deltat,fshort,flong,i_fstart,i_fend)
    ...
    if (is_mtm) then  
        ...
        !! misfit & adjoint 
        call mt_adj(s_tw,npts,deltat,nlen,df,i_fstart,i_fend,dtau_w,dlnA_w,w_taper,wp_taper,wq_taper,&
            err_dt_cc,err_dlnA_cc,&
        ...
        num=i_fend-i_fstart+1
            ...
            do  i = i_fstart,i_fend
            write(1,'(4e15.5)') fvec(i),dtau_w(i),tshift,err_dt_cc
            !print *,'dtau_w(i)=',dtau_w(i),'i=',i
            write(2,'(3e15.5)') fvec(i),dlnA_w(i),dlnA
            enddo
            close(1)
            close(2)
        ...
        if(COMPUTE_ADJOINT) then
           ...
           if( DISPLAY_DETAILS) then
                ...
                write( irec_str, '(I3)' )  irec
                write(filename,'(a)') &
                 trim(output_dir)//trim(adjustl(irec_str))//'_'//trim(adjustl(Tc_str))//'_adj_MT_win'

                open(1,file=filename,status='unknown')
                do  i =  i_tstart,i_tend
                write(1,'(I5,4e15.5)') i,adj(i),cc_max,sum(dtau_w(i_fstart:i_fend)*w_taper(i_fstart:i_fend))/sum(w_taper(i_fstart:i_fend)),err_dt_cc
                enddo
                close(1)
                close(2)
            endif
            ...
            !! averaged-mt shift:(no uncertainty estimates)
            !dtau_w_mean=sum(dtau_w(i_fstart:i_fend)*w_taper(i_fstart:i_fend))/sum(w_taper(i_fstart:i_fend))
            dtau_w_mean=sum(dtau_w(i_fstart:i_fend)*w_taper(i_fstart:i_fend))/sum(w_taper(i_fstart:i_fend))

            print *,'mean2 deltat fot MT is:',dtau_w_mean,'deltat fot CC is:',tshift
            print *,'cc_max=',cc_max,'num=',num,'err_dt_cc=',err_dt_cc
            print *,'misfit fot MT is:',misfit,'misfit fot CC is:',(tshift/err_dt_cc)**2
            ! tshift = averaged-mt shift
     !       tshift = dtau_w_mean
            ! ---------------------------------------------
            if ( (cc_max < CC_MIN) ) then
            ! zero the CC measurments
        ...
        if(DISPLAY_DETAILS) print*, 'CC (traveltime) misfit (s-d)'
        call CC_misfit(d,s,npts,deltat,tstart,tend,taper_percentage,taper_type, &
            'dt',compute_adjoint,adj,num,misfit,tshift,Tc,Te,irec,err_CC)
